...
export const EIGHT_LEVELS = [
  0, lum(0,0,255), lum(255,0,0), lum(255,0,255),
  lum(0,255,0), lum(0,255,255), lum(255,255,0), 255
];

export interface LevelInfo { name: string; gray: number; }

export const LEVEL_INFO: LevelInfo[] = ["Black","Blue","Red","Magenta","Green","Cyan","Yellow","White"]
  .map((name, i) => ({ name, gray: Math.round(EIGHT_LEVELS[i]) }));

export function hue2rgb(h: number): [number, number, number] {
  h = ((h % 360) + 360) % 360;
  const s = Math.min(5, Math.floor(h / 60)), f = (h % 60) / 60;
  return ([
...
function findPure(target: number): ColorCandidate[] {
  ...
  for (const f of formulas) {
    ...
    if (results.some(prev => Math.abs(prev.rgb[0]-c[0]) + Math.abs(prev.rgb[1]-c[1]) + Math.abs(prev.rgb[2]-c[2]) < PURE_DUPL_THRESHOLD)) continue;
...
export const DEFAULT_CC: number[] = LEVEL_CANDIDATES.map((alts, i) => {
  ...
  alts.forEach((x, j) => {
    const d = Math.abs(x.rgb[0]-CANONICAL_COLORS[i][0]) + Math.abs(x.rgb[1]-CANONICAL_COLORS[i][1]) + Math.abs(x.rgb[2]-CANONICAL_COLORS[i][2]);
