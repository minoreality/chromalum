 ...
 *
 * Model coordinates never use byte RGB. The hue hexagon uses channels in
 * 0..4 so every 15° GRB-tone intersection has integral coordinates.
 */
...
export type ChromalumChannels = readonly [number, number, number];
...
export function hueToChromalumChannels(hueAngleDeg: number): ChromalumChannels {
  ...
  return (
    [
      [CHROMALUM_CHANNEL_MAX, variableChannel, 0],
      [CHROMALUM_CHANNEL_MAX - variableChannel, CHROMALUM_CHANNEL_MAX, 0],
      [0, CHROMALUM_CHANNEL_MAX, variableChannel],
      [0, CHROMALUM_CHANNEL_MAX - variableChannel, CHROMALUM_CHANNEL_MAX],
      [variableChannel, 0, CHROMALUM_CHANNEL_MAX],
      [CHROMALUM_CHANNEL_MAX, 0, CHROMALUM_CHANNEL_MAX - variableChannel],
    ] as ChromalumChannels[]
  )[sector];
}

/** Recover a hue from a coordinate on the exact CHROMALUM hue hexagon. */
export function chromalumChannelsToHue([r, g, b]: ChromalumChannels): number {
...
export function chromalumGrbLevel([r, g, b]: ChromalumChannels): number {
  return (CHROMALUM_GRB_WEIGHTS.G * g + CHROMALUM_GRB_WEIGHTS.R * r + CHROMALUM_GRB_WEIGHTS.B * b) / CHROMALUM_CHANNEL_MAX;
}

export function chromalumToneNorm(channels: ChromalumChannels): number {
  return chromalumGrbLevel(channels) / CHROMALUM_TONE_DENOMINATOR;
}
