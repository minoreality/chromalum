...
export const ja: Record<TranslationKey, string> = {
  ...
  theory_binary_desc:
    "ここでは RGB 各チャンネルが 0 または 255 の8頂点だけを考え、色番号 0..7 を3ビット整数 4x₂ + 2x₁ + x₀ で表します。RGB の3チャンネルをこの3ビットへ割り当てる方法は6通りありますが、番号順に並べるとルマも昇順になることを要求すると、唯一の割当は bit2=Green, bit1=Red, bit0=Blue、つまり GRB です。これは Green のルマ係数が Red のルマ係数と Blue のルマ係数の和を超え、かつ Red のルマ係数が Blue のルマ係数を超えるためです（0.587 > 0.299 + 0.114, 0.299 > 0.114）。この割当は、人間の視覚感度に由来する BT.601 係数の不等式から一意に決まる帰結です。さらに、この割当の下では B=1, R=2, G=4 となり、原色番号は {1, 2, 4} です。この集合自体に独立した性質があります。元・ペア和・全体和（いずれも通常の整数加算）が重複なく {1, \u2026, 7} を埋め尽くす唯一の3元集合は、原色の番号 {1, 2, 4} に他なりません。",
  ...
  theory_dice_desc3:
    "階段状の展開図（2-2-2型）は色相環の順序に沿います。各ステップで 1 チャンネル切替 \u21d2 立方体 1 ビット隣接\u30fb連続するルマ順位\u30fb色相 60° 進行を同時に体現します。11種類の立方体展開図（回転・反転は同一視）のうち、この階段型だけが面隣接木（各面を頂点、辺共有を枝とする木）に色相環を辿る道 R→Y→G→C→B→M（またはその逆順）を含みます。証明の要点：立方体展開図は6面なので隣接関係を表す辺はちょうど 5 本であり、この5本の色相辺を要求した時点で面隣接木全体がそのハミルトン路に固定されます。その木に沿って立方体を展開すると、得られる展開図は 2-2-2 型の階段形に一意に定まります。",
  ...
  theory_conn_cube_geometry_hook:
    "立方体の幾何：ファノ平面の7線は、Black(0) を含む GF(2)³ の7つの2次元部分空間に対応します。ユークリッド的な立方体では、そのうち6つは実際の平面断面（3つの座標平面と3つの対角平面）として現れ、残る CMY 線は偶数パリティ四面体 {0,3,5,6} に対応します。座標平面 G=0, R=0, B=0 は、ハミング符号の3つのパリティ検査の核です。",
  ...
  theory_conn_extended:
    "全体パリティビットを担う 8 番目の座標（ラベル Black = 0）を加えると、この 7 点構造は [8,4,4] 拡張ハミング符号の 8 座標位置に埋め込めます。8 色そのものが符号語なのではなく、符号語の 8 つの座標位置のラベルです。",
