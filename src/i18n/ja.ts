...
export const ja = {
  ...
  theory_zigzag_desc:
    "色相を回転させると、GRB Binary Tone はどのように変わるのでしょうか。完全飽和色（最大チャンネル=1、最小=0）は、色相の回転に従って 6 つの区間からなるジグザグを描きます。各区間では1チャンネルが変化し、傾きの比は G:R:B = 4:2:1 になります。円周上では、各頂点 L1-L6 の水平線はジグザグと1点または3点で交わり、非頂点の水平線では最大 4 点で交わります。任意の色相角 h で T(h) + T(h+180°) = 1 が成り立つため、補色のトーン和は常に等しく 1 になります（補色定理）。",
  theory_dice_title: "カラーダイス",
  theory_dice_desc:
    "RGB 原色と CMY 二次色を GRB Binary Tone の小さい順に1～6とナンバリングし、六面体の面に配置します。補色写像 c ↦ c ⊕ 7 はトーン順を反転するため、補色ペアは対面に位置し、どの対面の和も7 — 標準的なサイコロの規則と同一です。",
  theory_dice_desc2:
  ...
  theory_conn_limit_vertices: "対象は、RGB 各チャンネルが 0 または 1 をとる8つの二値頂点だけです。連続色や中間値には別の座標系が必要です。",
  theory_conn_limit_tone: "GRB Binary Tone は4:2:1で定義したモデル内座標であり、知覚的明度、測光輝度、相対輝度ではありません。",
  theory_conn_limit_operations:
    "XOR は光の物理的な加法混色ではなく、AND は顔料の物理的な減法混色ではありません。補色もビット反転として定義しています。",
