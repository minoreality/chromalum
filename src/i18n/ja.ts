...
export const ja = {
  ...
  theory_binary_desc:
    "3つのチャンネルを 3 ビット整数 4x₂ + 2x₁ + x₀ の各ビット位置に割り当てる方法は6通りあります。そのうち、番号順と色の明るさ順が一致するのは、bit2=Green、bit1=Red、bit0=Blue、すなわち GRB の割当だけです（これは ITU-R BT.601 luma 係数の不等式 0.587 > 0.299 + 0.114、0.299 > 0.114 に基づく帰結です）。\n\nCHROMALUM の GRB Binary Tone Model では、この GRB 割当を採用し、RGB 各チャンネルが 0 または 1 の値をとる 8 頂点だけを扱います。各色を level = 4G + 2R + B で番号付けし、tone = level / 7 として正規化します。\n\nこの割当の下では B=1, R=2, G=4 となり、原色番号は {1, 2, 4} になります。この集合は、元・ペア和・全体和（いずれも通常の整数加算）が重複なく {1, …, 7} を埋め尽くす唯一の3元集合であり、この8色モデルにおける最小の2進基底そのものです。",
  ...
  theory_zigzag_desc:
    "色相を回転させると、色の明るさはどのように変わるのでしょうか。完全飽和色（最大チャンネル=1、最小=0）は、色相の回転に従って 6 つの区間からなるジグザグを描きます。各区間では1チャンネルが変化し、傾きの比は G:R:B = 4:2:1 になります。円周上では、各頂点 L1-L6 の水平線はジグザグと1点または3点で交わり、非頂点の水平線では最大 4 点で交わります。任意の色相角 h で T(h) + T(h+180°) = 1 が成り立つため、補色のトーン和は常に等しく 1 になります（補色定理）。",
  theory_dice_title: "カラーダイス",
  theory_dice_desc:
    "光の三原色と色の三原色を合わせた６つの色を暗い順に1～6とナンバリングし、六面体の面に配置します。補色写像 c ↦ c ⊕ 7 はトーン順を反転するため、補色ペアは対面に位置し、どの対面の和も7 — 標準的なサイコロの規則と同一です。",
  theory_dice_desc2:
