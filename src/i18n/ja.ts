...
export const ja: Record<TranslationKey, string> = {
  ...
  theory_venn_desc:
    "3つのチャンネル {G,R,B} の部分集合は 2\u00b3 = 8 通り存在します。3つの円が作る8領域が、そのまま8色に対応します。外側は空集合 ∅ (Black)、3つの円が重なる中央部は全集合 {G,R,B} (White) です。各色は部分集合 A \u2286 {G,R,B} と一対一に対応し、特性関数 \u03c7_A（A ∈ {G,R,B} なら 1、そうでなければ 0）として読めます。以下の全セクションは、この同じ8元集合を異なる角度から描いています。",
  ...
  theory_hamming_desc2:
    "Blue(1) は bit 0 が立つ位置 {1,3,5,7}、Red(2) は bit 1 が立つ位置 {2,3,6,7}、Green(4) は bit 2 が立つ位置 {4,5,6,7} を検査します。残りの色（M=3, C=5, Y=6, W=7）がデータ位置を担います。単一の位置に誤りが入ると、失敗したパリティ検査がその位置を特定します。この 3 ビットベクトル（シンドローム）が誤り位置の 2 進表現そのものです。",
