...
export const ja: Record<TranslationKey, string> = {
  ...
  theory_venn_desc:
    "3つのチャンネル {G,R,B} の部分集合は 2\u00b3 = 8 通り。3つの円が作る8領域が、そのまま8色に対応します。外側は空集合 ∅ (Black)、3つの円が重なる中央部は全集合 {G,R,B} (White) です。各色は部分集合 A \u2286 {G,R,B} と等価で、特性関数 \u03c7_A: {G,R,B} \u2192 {0,1} として読めます。以下の全セクションは、この同じ8元集合を異なる角度から描いています。",
  theory_binary_title: "バイナリレベル",
  theory_binary_desc:
    "ここでは RGB 各チャンネルが 0/255 の8頂点だけを考え、色番号 0..7 を3ビット数 4x₂ + 2x₁ + x₀ として使います。RGB の3チャンネルをこの3ビットへ割り当てる方法は6通りありますが、番号順 0,1,2,...,7 が BT.601 luma 順に単調増加することを要求すると、唯一の割当は bit2=Green, bit1=Red, bit0=Blue、つまり GRB です。これは Green のルマ係数が Red+Blue を超え、かつ Red が Blue を超えるためです（0.587 > 0.299 + 0.114, 0.299 > 0.114）。任意の設計ではなく、人間の視覚感度を反映した、BT.601 luma 係数の不等式から決まる帰結です。原色の番号 {1, 2, 4} は、元・ペア和・全体和（いずれも通常の整数加算）が {1, \u2026, 7} を重複なく生成する唯一の3元集合です。",
  ...
  theory_dice_desc:
    "6つの有彩色 RGB 頂点を暗い順に1\u20136とナンバリングし、六面体の面に配置します。補色写像 c \u21a6 c xor 7 はルマ順を反転するため、補色ペアは対面に位置し、どの対面の和も7 \u2014 標準的なサイコロの規則と同一です。",
  theory_dice_desc2:
    "これは BT.601 に限らず一般化できる定理です：任意の正のルマ係数に対し、ルマ値にタイがなければ、補色の順序反転性から d(c) + d(c\u0304) = 7 が成立します。",
  ...
  theory_cube_desc:
    "8レベルは3次元立方体（RGB軸）の頂点に対応します。各辺は1チャンネルの切り替えです。Black(0) と White(7) を両極とし、6つの有彩色頂点が赤道帯を形成します。赤道帯は、重み1の原色層と重み2の二次色層から成ります。Black(0) を基準にすると原色の組み合わせはXORとして読めます（加法的な読み）。White(7) を基準にするとCMYの組み合わせはANDとして読めます（減法的な読み）。補色写像（XOR 7）がこの2つの視点を入れ替えます。",
  theory_cube_desc2:
    "「ハッセ図」ボタンで、同じ立方体を体対角線方向から眺めた射影に切り替わります（同じ Q\u2083 グラフ）。K を下端、W を上端とし、ランク = |A|（色が含むチャンネル数）で 4 層に分かれます \u2014 パスカル三角形の第3行 1, 3, 3, 1 が直接読めます。原子（rank 1 = RGB 原色）と余原子（rank 2 = CMY）は補写像で対応し、これが補色対の代数的起源です。12 本の辺は G・R・B チャンネルごとの 3 つの完全マッチングに分解されます。上向きに辿れば加法混色、下向きなら減法混色 \u2014 ハッセ図の自己双対性がこの加減法の等価性を与えています。",
  ...
  theory_octa_desc:
    "6つの有彩色とその補色対称性を最も対称的に表す図形が八面体です。6頂点は B, R, M, G, C, Y に対応し、3本の対頂点軸 B↔Y、R↔C、M↔G は補色写像 c ↦ c⊕7 を表します。これは知覚均等色空間ではなく、RGBチャンネルのオン/オフから得られる離散的な補色対称性を幾何化したものです。",
  theory_octa_desc2:
    "八面体の12辺は、補色ではない有彩色ペアだけを結びます。各辺 {a,b} には第三の有彩色 a⊕b が定まり、12辺全体で6つの有彩色 XOR 結果が各2回ずつ現れます。さらに、八面体の8面は3本の補色軸から片側を1つずつ選ぶ8通りに対応し、それぞれが GF(2)³ の8元を表します。この面隣接グラフは立方体グラフそのものであり、この意味でカラーダイヤはカラーキューブの双対です。",
