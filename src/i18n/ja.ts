...
export const ja = {
  ...
  theory_binary_desc:
    "ここでは、RGB 各チャンネルが 0 または 1 の 8 頂点だけを扱い、各色を3ビット整数 4x₂ + 2x₁ + x₀ として番号付けし、レベル 0..7 を定義します。\n\nGRBバイナリトーンモデルでは、bit2=Green, bit1=Red, bit0=Blue とし、level = 4G + 2R + B、tone = level / 7 と定義します。ビット重みは 4:2:1 なので、外部の輝度規格を導入せずに、番号順とトーン順が一致します。\n\nこの割当の下では B=1, R=2, G=4 となり、原色番号は {1, 2, 4} になります。この集合は、元・ペア和・全体和（いずれも通常の整数加算）が重複なく {1, …, 7} を埋め尽くす唯一の3元集合であり、この8色モデルにおける最小の2進基底そのものです。",
  ...
  theory_zigzag_desc:
    "色相を回転させると、トーンはどのように変わるのでしょうか。完全飽和色（最大チャンネル=1、最小=0）は、色相の回転に従って 6 つの区間からなるジグザグを描きます。各区間では1チャンネルが変化し、GRBバイナリトーンでは傾きの比が G:R:B = 4:2:1 になります。円周上では、各頂点トーン値の水平線はジグザグと1点または3点で交わります。任意の色相角 h で T(h) + T(h+180°) = 1 が成り立つため、補色のトーン和は常に等しくなります。",
  theory_dice_title: "カラーダイス",
  theory_dice_desc:
    "光の三原色と色の三原色を合わせた６つの色を GRBバイナリトーンの低い順に1\uff5e6とナンバリングし、六面体の面に配置します。補色写像 c \u21a6 c \u2295 7 はトーン順を反転するため、補色ペアは対面に位置し、どの対面の和も7 \u2014 標準的なサイコロの規則と同一です。",
  theory_dice_desc2:
