...
export const ja = {
  ...
  hex_luma_sequence: "一周のルマ列: 2,3,4,5,6,5,4,5,4,3,2,1,2,3",
  ...
  theory_binary_desc:
    "ここでは、RGB 各チャンネルが 0 または 255 の 8 頂点だけを扱い、各色を3ビット整数 4x₂ + 2x₁ + x₀ として番号付けし、レベル 0..7 を定義します。\n\nRGB の3チャンネルを x₂, x₁, x₀ に割り当てる方法は6通りあります。このうち、番号順と BT.601 luma 順が一致する割当は、bit2=Green, bit1=Red, bit0=Blue、つまり GRB だけです。理由は、上位ビットの重みが下位2ビットの重みの和を超え、かつ中位ビットの重みが下位ビットの重みを超えるためです。これは、この有限な RGB 頂点モデルにおける BT.601 luma 係数の不等式（0.587 > 0.299 + 0.114, 0.299 > 0.114）から従う帰結です。\n\nこの割当の下では B=1, R=2, G=4 となり、原色番号は {1, 2, 4} になります。この集合は、元・ペア和・全体和（いずれも通常の整数加算）が重複なく {1, …, 7} を埋め尽くす唯一の3元集合であり、この8色モデルにおける最小の2進基底そのものです。",
  ...
  theory_zigzag_desc:
    "色相を回転させると、明るさはどのように変わるのでしょうか。完全飽和色（最大チャンネル=255、最小=0）は、色相の回転に従って 6 つの区間からなるジグザグを描きます。各区間では1チャンネルが変化し、傾きの絶対値は BT.601 係数に比例します。円周上では、各頂点ルマ値の水平線はジグザグと1点または3点で交わり、その交点が同じルマを持つ色相です。非頂点ルマ値では、水平線はジグザグと最大 4 点で交わります。任意の色相角 h で Y(h) + Y(h+180\u00b0) = 255 が成り立つため、補色のルマ和は常に等しい（= 255）です（補色ルマ定理）。これは係数の和が 1 である任意の luma 定義で成立します。",
  theory_dice_title: "カラーダイス",
  theory_dice_desc:
    "光の三原色と色の三原色を合わせた６つの色を暗い順に1\uff5e6とナンバリングし、六面体の面に配置します。補色写像 c \u21a6 c \u2295 7 はルマ順を反転するため、補色ペアは対面に位置し、どの対面の和も7 \u2014 標準的なサイコロの規則と同一です。",
  theory_dice_desc2:
    "面配置を d: c \u2192 {1, ..., 6}、補色を c\u0304 = c \u2295 7 と書きます。これは BT.601 に限らず一般化できる定理です：任意の正のルマ係数に対し、ルマ値に重複がなければ、補色の順序反転性から d(c) + d(c\u0304) = 7 が成立します。",
  theory_dice_net_title: "\u30ab\u30e9\u30fc\u30c0\u30a4\u30b9\u306e\u8272\u76f8\u5c55\u958b",
  theory_dice_desc3:
    "階段状の展開図（2-2-2型）は色相環の順序に沿います。各ステップで 1 チャンネル切替 \u21d2 立方体 1 ビット隣接\u30fb色相 60° 進行\u30fbBT.601 ルマのジグザグを同時に体現します。11種類の立方体展開図（回転・反転は同一視）のうち、この階段型だけが面隣接木（各面を頂点、辺共有を枝とする木）に色相環を辿る道 R→Y→G→C→B→M（またはその逆順）を含みます。証明の要点：立方体展開図は6面なので隣接関係を表す辺はちょうど 5 本であり、この5本の色相辺を要求した時点で面隣接木全体がそのハミルトン路に固定されます。その木に沿って立方体を展開すると、得られる展開図は 2-2-2 型の階段形に一意に定まります。",
  ...
  theory_gray_desc:
    "6つの有彩色を色相環の順に並べると、各ステップで1チャンネルだけが切り替わります：G, R, B の順で 2 周し、六角形の巡回路を形成します。構造的には、6頂点は K\u2083,\u2083（原色\u2194二次色）から、完全マッチングを成す補色ペア3組を除いた形です。その結果、この六角形だけが残り、他の辺は存在しません。各辺は 2 つの原色を含むファノ平面の三点線上にあり、3本の線を各2回ずつ使います。補色は常に対蹠（ハミング距離 3、六角形上で半周＝3 ステップ）であり、これがダイスの対面の和が7になる幾何学的理由です。",
  ...
  theory_hamming_desc:
    "ここで使う位置 1..7 は、ファノ平面の7点と同じ非零3ビットラベルです。標準的な Hamming(7,4) の座標配置では、2の冪である B=1, R=2, G=4 がパリティビット位置であり、同時に bit 0, bit 1, bit 2 の3本のパリティ検査行を表すラベルになります。",
  ...
  theory_connections_desc:
    "GF(2)³ は、RGB 0/255 の8頂点をまとめる共通のベクトル空間です。ここから複数の数学的構成が現れます。生成元 {B,R,G} = {1,2,4} を選ぶと、1ビット差のケイリーグラフはカラーキューブ Q₃ になります。零点 Black(0) を除く7点を射影化するとファノ平面 PG(2,2) になり、同じ7つの非零ベクトルは Hamming(7,4) のパリティ検査行列の列としても読めます。\n\n有彩色6点だけを見ると、R→Y→G→C→B→M→R は1ビット反転のグレイコード巡回です。また、8点を {G,R,B} の部分集合として見れば、XOR は対称差、AND は共通部分となり、ブール環の構造が得られます。したがって、本タブの各図は別々の理論の並置ではなく、同じ8色モデルを代数・幾何・符号の視点から読み替えたものです。",
  ...
  theory_octa_desc:
    "6つの有彩色とその補色対称性を最も対称的に表す図形が八面体です。6頂点は B, R, M, G, C, Y に対応し、3本の対頂点軸 B↔Y、R↔C、M↔G は補色写像 c ↦ c⊕7 を表します。これはRGBチャンネルのオン/オフから得られる離散的な補色対称性を幾何化したものです。",
  theory_octa_desc2:
    "八面体の12辺は、補色ではない有彩色ペアだけを結びます。各辺 {a,b} には第三の有彩色 a⊕b が定まり、12辺全体で6つの有彩色 XOR 結果が各2回ずつ現れます。",
  ...
  stats_map_diversity: "多様度",
  stats_map_isolation: "孤立",
  stats_map_boundaryDistance: "境界距離",
  stats_map_gradient: "階調勾配",
  stats_map_levelTone: "階調",
  stats_map_colorLuma: "色階調",
  stats_map_region: "連結領域",
  ...
  music_luma_title: "対称性",
  music_luma_sym: "Flat",
  music_luma_bt601: "Luma",
  music_stop_all: "全停止",
  music_reset: "\u30ea\u30bb\u30c3\u30c8",
  music_mute: "\u30df\u30e5\u30fc\u30c8",
