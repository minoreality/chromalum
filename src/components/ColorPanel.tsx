...
export const ColorPanel = React.memo(function ColorPanel(props: ColorPanelProps) {
  const {
    prvRef, prvCurRef, prvWrapRef, displayW, displayH, canvasTransform, canvasCursor,
    cc, ccDispatch, brushLevel, setBrushLevel, tool,
    panZoom, drawing,
  } = props;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: SP.lg }}>
      <div style={{ fontSize: FS.md, color: C.textDim, minHeight: SP["3xl"] }}>
        {t("label_colorized")}
      ...
      <div ref={prvWrapRef} tabIndex={0}
        ...
        style={{ border: `1px solid ${C.border}`, borderRadius: R.lg, overflow: "hidden", position: "relative", width: displayW, height: displayH }}>
        <canvas ref={prvRef}
        ...
        />
        <canvas ref={prvCurRef} width={displayW} height={displayH}
          style={{ position: "absolute", top: 0, left: 0, width: displayW, height: displayH, pointerEvents: "none", zIndex: Z.cursorOverlay }} />
      </div>
      <div style={{ fontSize: FS.md, color: C.textDimmer }}>{t("label_color_mapping")}</div>
