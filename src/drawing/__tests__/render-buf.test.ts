...
describe("renderCanvasBuffers", () => {
  ...
  beforeEach(() => {
    lut = buildColorLUT(DEFAULT_CANDIDATE_INDEX_BY_LEVEL);
  });
  ...
  it("returns early when both canvases are null", () => {
    ...
    expect(cache.previewImageData).toBeNull();
  });
  ...
  it("creates ImageData cache on first call", () => {
    ...
    expect(cache.sourceImageData).not.toBeNull();
    expect(cache.sourceImageData!.width).toBe(w);
    expect(cache.sourceImageData!.height).toBe(h);
  });
  ...
  it("recreates ImageData when canvas size changes", () => {
    ...
    const firstSrc = cache.sourceImageData;
    expect(firstSrc).not.toBeNull();

    // Now render with different dimensions
    const data8 = new Uint8Array(64);
    renderCanvasBuffers(data8, 8, 8, lut, srcCanvas, null, cache);
    expect(cache.sourceImageData).not.toBe(firstSrc);
    expect(cache.sourceImageData!.width).toBe(8);
  });
  ...
  it("writes correct gray values for level 0 (black)", () => {
    ...
    const s32 = new Uint32Array(cache.sourceImageData!.data.buffer);
  ...
  it("applies color LUT correctly to preview canvas", () => {
    ...
    const p32 = new Uint32Array(cache.previewImageData!.data.buffer);
    const rgb = lut[2];
  ...
  describe("dirty rect rendering", () => {
    it("only updates pixels within dirty rect bounds", () => {
      ...
      renderCanvasBuffers(data0, w, h, lut, srcCanvas, null, cache);

      // Second render: all level 7 but only dirty rect at (1,1,2,2)
      renderCanvasBuffers(data, w, h, lut, srcCanvas, null, cache, { x: 1, y: 1, w: 2, h: 2 });
    ...
    it("clamps dirty rect to canvas bounds", () => {
      ...
      expect(() => {
        renderCanvasBuffers(data, w, h, lut, srcCanvas, null, cache, { x: -1, y: -1, w: 10, h: 10 });
      }).not.toThrow();
    });
    ...
    it("skips rendering when dirty rect has zero area", () => {
      ...
      const s32Before = new Uint32Array(cache.sourceImageData!.data.buffer.slice(0));

      // Dirty rect with zero width — should not update
      renderCanvasBuffers(data.fill(7), w, h, lut, srcCanvas, null, cache, { x: 5, y: 5, w: 1, h: 1 });
      const s32After = new Uint32Array(cache.sourceImageData!.data.buffer);
