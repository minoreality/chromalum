...
export function paintLine(
  ...
  const sx = x0 < x1 ? 1 : -1,
    sy = y0 < y1 ? 1 : -1;
  let e = ax - ay;
  // Sparse optimization: skip paintCircle when within r/4 of last painted point
...
export function paintEllipse(
  ...
  const rx = Math.abs(x1 - x0) >> 1,
    ry = Math.abs(y1 - y0) >> 1;
  if (rx === 0 && ry === 0) {
    paintCircle(data, cx, cy, r, lv, w, h);
    return;
  }
  if (rx === 0) {
    paintLine(data, cx, cy - ry, cx, cy + ry, r, lv, w, h);
    return;
  }
  if (ry === 0) {
    paintLine(data, cx - rx, cy, cx + rx, cy, r, lv, w, h);
    return;
  }
  let x = 0,
    y = ry;
