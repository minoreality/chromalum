...
export const HEX_CANDIDATE_ANGLES: (number | null)[][] = LEVEL_CANDIDATES.map((alts) => alts.map(() => null));
HEX_VERTICES.forEach((v, i) => {
  HEX_CANDIDATE_ANGLES[v.lv][HEX_VERTEX_ALTS[i]] = HEX_ANGLES[i];
});
HEX_EDGES.forEach((e, ei) => {
  e.lv.forEach((lv, li) => {
    HEX_CANDIDATE_ANGLES[lv][HEX_EDGE_ALTS[ei][li]] = HEX_EDGE_COLORS[ei][li].hue;
  });
});
