...
1. `level = 4G + 2R + B` の GRB Binary Tone 順序を中核座標として固定すること。
...
> CHROMALUM は、既知の `GF(2)^3` 色加算、RGB cube、Boolean lattice、Fano 平面、Hamming 符号、多面体構造を基礎に、GRB Binary Tone レベル順序、補色-dice 定理、hue/tone/dice/polyhedra の対応を統合した、8 頂点 RGB 色集合の離散代数的色彩アトラスである。
...
CHROMALUM 側の差分は、`B3` を GRB Binary Tone レベル順序、補色-dice、Fano/Hamming、K8 分解へ接続する点にある。
...
CHROMALUM 側の差分は、Taylor の色加算遊びを RGB display primaries、GRB Binary Tone、Hamming syndrome labels、多面体分解と同じ UI に統合する点である。
...
GRB Binary Tone は、外部の明るさ規格ではなく、CHROMALUM が 8 頂点 RGB を読むための正準座標である。
...
この定義により、bit significance は `G,R,B` の順になり、`lv = 4G + 2R + B` が tone 順と一致する。ここは「既知の輝度係数による導出」ではなく、既知構造を統合するための CHROMALUM 側の命名・正規化である。

### Dice / Cube Net / Stella

標準サイコロの対面和 7、cube nets、stella octangula は既知である。CHROMALUM 側の主張は、補色 tone 反転から die rank sum 7 が出ること、さらに hue Gray path が 2-2-2 staircase net と結びつくことである。
...
4. Hamming 1950 / Fano/Hamming 標準資料: Hamming code の原典、`PG(2,2)` と Hamming(7,4) の既知対応。
5. MathWorld Cube / Tetrahedron 2-Compound: cube nets と stella 背景。
...
- R. W. Hamming, "Error Detecting and Error Correcting Codes", Bell System Technical Journal 29(2), 147-160, 1950. https://doi.org/10.1002/j.1538-7305.1950.tb00463.x
