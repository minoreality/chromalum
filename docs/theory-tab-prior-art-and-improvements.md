...
改善前の日本語 UI では `color_detail_luma` が「輝度」になっていたため、luma と luminance/lightness の混同リスクがあった。現在は `ルマ` / `Luma` に寄せている。また、旧 `theory_binary_desc` の「人間の色覚の帰結」は強すぎたため、現在は「BT.601 luma 係数の不等式からの帰結」としている。
...
Theory タブの AND / CMY reading は「物理的な顔料混合」ではなく、「ブール束上の理想化された meet」として表示すべきである。改善前のセクション見出し「XOR 混色」は誤読されやすかったため、現在は「XOR 演算」に変更している。

## Claim Risk Assessment

The first three high-risk copy issues have been implemented. The remaining entries are retained as backlog and review criteria.
...
### P0: Claim Hygiene (Implemented)

Implemented changes:

1. Theory タイトルを `色彩理論` から `離散代数的色彩理論` へ変更した。
2. `label_theory` は `ALGEBRAIC COLOR THEORY` のまま維持し、H2 で範囲を限定した。
3. `color_detail_luma: "輝度"` を `ルマ` に変更した。
4. `theory_binary_desc` を BT.601 luma 係数の不等式に基づく説明へ変更した。
...
1. `AG(3,2)` affine planes explorer with a local affine-plane data table added when the explorer is implemented.
...
| Area | Status | Notes |
| --- | --- | --- |
| Rename `theory_title` | Done | UI now uses `Discrete Algebraic Color Theory` / `離散代数的色彩理論`. |
| Luma terminology | Done | UI uses `Luma` / `ルマ` rather than luminance/輝度 for BT.601 luma. |
| Human-vision wording | Done | Binary-level copy now ties the ordering to BT.601 luma coefficient inequalities in this finite model. |
| Scope card | Done | Theory tab includes a Scope and Limits summary. |
| Dice-net rigor | Done | Cube-net enumeration and hue-path adjacency are machine-checked in `src/data/__tests__/theory-data.test.ts`. |
| References in app UI | Deferred | README/docs carry the bibliography; an in-app references drawer remains optional. |
| Coefficient comparison | Remaining | BT.709/WCAG/Oklab comparison is not implemented. |
| Advanced explorers | Deferred | `AG(3,2)`, broader `GL(3,2)`, and proof-audit panels should be added with their data when implemented. |
...
| --- | --- | --- |
| `theory_title` | Done | `離散代数的色彩理論` |
| `color_detail_luma` | Done | `ルマ` |
| `theory_binary_desc` | Done | `BT.601 luma 係数の不等式` |
| `theory_xor_title` | Done | `XOR 演算` |
...
| Key | Current | Proposed |
| --- | --- | --- |
| `theory_title` | Done | `Discrete Algebraic Color Theory` |
| `theory_binary_desc` | Done | `BT.601 luma coefficient inequalities` |
| `theory_xor_title` | Done | `XOR Operation` |
