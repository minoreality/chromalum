...
この区分線形関数を `L(theta) = 7T(theta)` と書くと、三角関数的な座標系に対して次の構造を持つ。
...
```text
c(theta) = GRB(G4,R4,B4),  0 <= G4,R4,B4 <= 4
```
...
R/C 構成では、L2 等トーン線 `R-P225` と、その補色像である L5 等トーン線 `C-P45` が平行になる。`R` から `C-P45` へ下ろした垂線の足を `F` とすると、正確な座標は
...
となるため、`R-F-C-(-F)` は M/G 長方形と同じ単位外接円に内接する長方形である。ただし辺比は
...
を同じ円上で比較する。R から時計回りに測った中心角を `theta` とすれば、

```text
sin(theta) = 3*sqrt(3)/14
cos(theta) = 13/14
theta ~= 21.786789deg
```
...
```text
sin(2*theta) = 2 sin(theta) cos(theta) = 39*sqrt(3)/98
cos(2*theta) = cos(theta)^2 - sin(theta)^2 = 71/98
```

となり、これはちょうど `F` の座標に一致する。したがって外接円上の順序は

```text
R(0) -> X(theta) -> F(2*theta)
C(180deg) -> Z(180deg+theta) -> -F(180deg+2*theta)
```

であり、`X` と `Z=-X` はそれぞれ隣接する 2 点の厳密な円弧中点である。弦長も

```text
|R-X|^2 = |X-F|^2 = 2 - 2*cos(theta) = 1/7
|C-Z|^2 = |Z-(-F)|^2 = 1/7
```
...
に等しい。この `theta` は色相閉路の `15deg` 格子や `22.5deg` 分割そのものではなく、二つの異なる等トーン作図を共通外接円上で結び付ける創発的な代数角である。
...
以上から、CHROMALUM の図形構造は少なくとも次の 2 層に分けられる。

- **代数・アフィン層:** GRB `4:2:1`、level 反転 `L -> 7-L`、`L3.5` 共通平均、中心反転、相補平行性。
- **ユークリッド計量層:** 直角、厳密な `60deg`、円内接、弦長、倍角、円周上の等間隔。

第一層は補色写像そのものから体系的に導かれる。第二層は正六角形埋め込みと特定の等トーン線の組合せに依存する。特に、M/G の直交性、L2/L4 の `60deg`、M/G と R/C を結ぶ倍角関係が同じ `Q(sqrt(3))` 座標上で同時に成立することは、GRB Binary Tone の離散代数と正六角形のユークリッド幾何が交差する箇所として研究上重要である。

第三に、Music タブの 12 平均律 2 オクターブ写像では、1 周 `2π` が 24 半音に対応するため、`π/12` は 1 半音に対応する。したがって、上の交点角は音高写像上では半音格子に乗る。
