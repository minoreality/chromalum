...
画像入力だけは別経路である。入力された sRGB コード値へモデル固有の 4:2:1 スコアを適用し、最寄りの `L0..L7` ラベルを推定する。この処理は非可逆な内部分類器であり、`λ` の逆写像でも、`H` 上の候補座標や色相角の復元でもない。また、この入力側スコアは GRB Binary Tone、知覚的明度、測光輝度のいずれとも同一視しない。
